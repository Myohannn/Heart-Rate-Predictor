...
class RFClassifierTrainer:
    ...
    def load_data(self):
        ...
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=float(conf.get("Model_Trainer",
                                                                                           "clf_test_size")),
                                                            random_state=0,
                                                            shuffle=True)
    ...
    def biuld_model(self, datas):
        clf = RandomForestClassifier(n_estimators=int(conf.get("Model_Trainer", "clf_n_estimators")))
...
class RFPredictorTrainer:
    ...
    def run(self):
        ...
        X_train, X_test, y_train, y_test = train_test_split(X, Y, test_size=float(conf.get("Model_Trainer",
                                                                                           "RFpredictor_test_size")),
                                                            random_state=42, shuffle=True)

        print("Biulding model......\n")
        rf_model = RandomForestRegressor(n_estimators=int(conf.get("Model_Trainer", "RFpredictor_n_estimators")))
...
class KerasNNPredictorTrainer:
    ...
    def run(self):
        ...
        # config file
        units_Array = conf.get("Model_Trainer", "units_Array").split(",")
        layers_Array = conf.get("Model_Trainer", "layers_Array").split(",")
        batch_Array = conf.get("Model_Trainer", "batch_Array").split(",")
        epoch_Array = conf.get("Model_Trainer", "epoch_Array").split(",")
            ...
            for layers in layers_Array:
                for batch in batch_Array:
                    for epoch in epoch_Array:
                        self.train_model(int(units), int(layers), int(batch), int(epoch))
